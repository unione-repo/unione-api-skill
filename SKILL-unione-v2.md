...
**API Endpoints** (choose based on user's account region):
- **EU:** `https://eu1.unione.io/en/transactional/api/v1`
- **US/Canada:** `https://us1.unione.io/en/transactional/api/v1`

Default to **EU** endpoint unless the user specifies otherwise. All methods use `POST` with JSON body.

> **Important:** If you are unsure which region the user's account belongs to, ask them. Sending requests to the wrong region will return authentication errors. If the user doesn't know, try EU first — if you get a 401, switch to US.
...
### Step 1: Get Required DNS Records — `domain/get-dns-records.json`

```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/domain/get-dns-records.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
  -d '{"domain": "yourdomain.com"}'
```

This returns the DNS records (DKIM, SPF, etc.) that the user must add to their DNS provider. **Present these records clearly to the user and instruct them to add them at their DNS provider (Cloudflare, Route53, GoDaddy, etc.).**
...
```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/domain/validate-verification.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
  -d '{"domain": "yourdomain.com"}'
```
...
```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/domain/validate-dkim.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
  -d '{"domain": "yourdomain.com"}'
```
...
```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/domain/list.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
  -d '{}'
```
...
```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/email/send.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
  -d '{
...
```javascript
const response = await fetch("https://eu1.unione.io/en/transactional/api/v1/email/send.json", {
  method: "POST",
  headers: {
...
```php
$ch = curl_init("https://eu1.unione.io/en/transactional/api/v1/email/send.json");
...
```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/email-validation/single.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
...
```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/template/set.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
  -d '{
    "template": {
...
```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/template/get.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
...
```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/template/list.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
  -d '{"limit": 50, "offset": 0}'
```
...
```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/template/delete.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
  -d '{"id": "template-id-here"}'
```
...
```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/webhook/set.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
  -d '{
...
```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/webhook/list.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
  -d '{}'
```
...
```bash
# Get
curl -X POST ".../webhook/get.json" -H "X-API-KEY: $UNIONE_API_KEY" \
  -H "Content-Type: application/json" -d '{"url": "https://yourapp.com/unione-webhook"}'

# Delete
curl -X POST ".../webhook/delete.json" -H "X-API-KEY: $UNIONE_API_KEY" \
  -H "Content-Type: application/json" -d '{"url": "https://yourapp.com/unione-webhook"}'
```
...
```bash
curl -X POST "https://eu1.unione.io/en/transactional/api/v1/suppression/set.json" \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: $UNIONE_API_KEY" \
...
```bash
curl -X POST ".../suppression/get.json" -H "X-API-KEY: $UNIONE_API_KEY" \
...
```bash
curl -X POST ".../suppression/list.json" -H "X-API-KEY: $UNIONE_API_KEY" \
...
```bash
curl -X POST ".../suppression/delete.json" -H "X-API-KEY: $UNIONE_API_KEY" \
  -H "Content-Type: application/json" -d '{"email": "user@example.com"}'
```
...
```bash
curl -X POST ".../event-dump/create.json" -H "X-API-KEY: $UNIONE_API_KEY" \
  -H "Content-Type: application/json" \
...
curl -X POST ".../event-dump/get.json" -H "X-API-KEY: $UNIONE_API_KEY" \
  -H "Content-Type: application/json" -d '{"dump_id": "dump-id"}'

# List all dumps
curl -X POST ".../event-dump/list.json" -H "X-API-KEY: $UNIONE_API_KEY" \
  -H "Content-Type: application/json" -d '{}'

# Delete a dump
curl -X POST ".../event-dump/delete.json" -H "X-API-KEY: $UNIONE_API_KEY" \
  -H "Content-Type: application/json" -d '{"dump_id": "dump-id"}'
```
...
```bash
# List tags
curl -X POST ".../tag/list.json" -H "X-API-KEY: $UNIONE_API_KEY" \
  -H "Content-Type: application/json" -d '{}'

# Delete tag
curl -X POST ".../tag/delete.json" -H "X-API-KEY: $UNIONE_API_KEY" \
...
```bash
# Create project
curl -X POST ".../project/create.json" -H "X-API-KEY: $UNIONE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"project": {"name": "My Project", "send_enabled": true}}'

# List projects
curl -X POST ".../project/list.json" -H "X-API-KEY: $UNIONE_API_KEY" \
  -H "Content-Type: application/json" -d '{}'
```
...
```bash
curl -X POST ".../system/info.json" -H "X-API-KEY: $UNIONE_API_KEY" \
  -H "Content-Type: application/json" -d '{}'
```
...
```bash
curl -X POST ".../email/subscribe.json" -H "X-API-KEY: $UNIONE_API_KEY" \
  -H "Content-Type: application/json" \
...
2. **Always ask the user which region** their UniOne account is in (EU or US) if not already known. Default to EU (`eu1.unione.io`). If you get a 401 error, try the other region.
